...
## copy from bi-dexhands/algorithms/marl/runner.py:5
def _t2n(x):
...
class MAPPO_Runner:
    ...
    def train(self):
        ...
        for agent_id in torch.randperm(self.num_agents):
            ...
            action_prod = torch.exp(
                (new_actions_logprob.detach() - old_actions_logprob.detach()).reshape(self.episode_length,
                                                                                      self.n_rollout_threads,
                                                                                      action_dim).sum(dim=-1,
                                                                                                      keepdim=True))
